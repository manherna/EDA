...
public:
	...
	int tallaArbol() {
		return cuentaNodos(_ra);
	}
	std::string frontera() {
		return calculaFrontera(_ra);
	}
	int get_minimum() {
		if (_ra == nullptr)return -1;
		else return minelem(_ra);
	}
	...
	int numNodos (funcion f) {
		return numNodos(_ra, f);
	}
...
private: 
	...
	*/
	...
	std::string calculaFrontera(Nodo * ra) {
		if (ra == nullptr) return "";
		else {
			if (ra->_iz != nullptr || ra->_dr != nullptr) {
				string aux1 = calculaFrontera(ra->_dr);
				string aux2 = calculaFrontera(ra->_iz);
				return aux1 + aux2;
			}
			else {
				string s = "";
				s.push_back(ra->_elem);
				return s;
			}
		}
	}
	int cuentaNodos(Nodo * ra){
		if (ra != nullptr) {
		int tamiz = 0;
		int tamder = 0;
		if (ra->_iz != nullptr)
			tamiz = cuentaNodos(ra->_iz);
		if (ra->_dr != nullptr)
			tamder = cuentaNodos(ra->_dr);

		return (tamiz > tamder) ? tamiz + 1 : tamder + 1;
	}
		else return 0;
	}
	int minelem(Nodo * ra) {
		...
		aux3 = INT32_MAX;
		aux1 = ra->_elem;
		if (ra->_iz != nullptr)aux2 = minelem(ra->_iz);
		if (ra->_dr != nullptr)aux3 = minelem(ra->_dr);

		if (aux3 < aux2)aux2 = aux3;
	...
	template <class funcion>
	int numNodos (Nodo* ra, funcion f) {
		if (ra == nullptr) return 0;
		else {
			if (f(ra->_elem)) return 1 + numNodos(ra->_iz, f) + numNodos(ra->_dr, f);
			else return numNodos(ra->_iz, f) + numNodos(ra->_dr, f);
		}
	}
